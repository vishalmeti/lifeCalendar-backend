...
exports.generateSummaryForEntry = async (req, res) => {
  try {
    ...
    // Update the entry with the newly generated summary
    entry.summary = summaryText;
    await entry.save(); // Save the new summary to the database

    // Return the updated entry (which includes the new summary)
    res.json({
